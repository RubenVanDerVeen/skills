...
- Adding a level-2 heading around `#include "subdocs/foo.typ"` when the subdoc already starts with `= Foo` (level 1). The two headings collide and `chapter-numbering` falls out of sync. Convention: the subdoc's `=` is the chapter title of the bijlage; in main.typ, write `#include "subdocs/foo.typ"` directly (no surrounding heading), or fold the label into the subdoc's first heading (`= Bijlage A: Foo`).
...
- Using em-dash (U+2014) or sentence-level hyphen (`-`) in body prose. Use `.`, `;`, or `,` instead. Hyphens in compound words and identifiers are fine.
...
Pkg isn't installed in the local data dir. Source lives at `C:\Users\ruben\Projects\Tools\TypstTools\`. Copy or symlink to the Typst local packages directory. On Windows: `%LOCALAPPDATA%\typst\packages\local\typst-tools\<X.Y.Z>\` (the path is `LOCALAPPDATA`, not `APPDATA`; the user Roaming dir is for app settings, Typst uses Local). On Linux: `~/.local/share/typst/packages/local/typst-tools/<X.Y.Z>/`.

One-time setup (Windows, PowerShell):

```powershell
$src = "C:\Users\ruben\Projects\Tools\TypstTools"
$dst = "$env:LOCALAPPDATA\typst\packages\local\typst-tools\0.1.8"
New-Item -ItemType Directory -Force -Path (Split-Path $dst) | Out-Null
New-Item -ItemType Junction -Path $dst -Target $src | Out-Null
```

Pick the version you want to pin (the folder name is the version, e.g. `0.1.8`). If a `<X.Y.Z>` is already present (as a real dir or junction), `New-Item -ItemType Junction` will fail; remove it first or bump the version.
