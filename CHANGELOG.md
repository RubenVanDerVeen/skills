...
- `deep-research/`: end-to-end research pipeline skill (Hermes research profile). Intake â†’ parallel gather (arxiv + web + own vault) â†’ synthesized dossier with citations â†’ brainstorm or Typst draft. Catalogue rows added in `README.md` (Skills table + Layout block) and `AGENTS.md` (Current skills table).
...
- `AGENTS.md`: branch model `master` â†’ `main`; added `synctool-sync` row to the Current skills table.
- `typst-pro`: bumped `@local/typst-tools` baseline `0.1.2` â†’ `0.1.8` across skill examples, imports, and install paths. Factual references (fixed-in bug notes, rename reset point, version-numbering example, `@preview` pins) kept at `0.1.2`.
...
- `docs/workflow.md`: Large feature section documents the full multi-plan flow (outline approval â†’ per-part specs/plans â†’ manifest handoff â†’ fresh session per dispatch prompt, integration never returns to the planner). Snapshot 2026-07-17.
