...
- feat(skills): `vendored brainstorming from superpowers, tailored to repo conventions.` Design-first gate with batched questions and native spec path.
- feat(skills): `vendored writing-plans from superpowers, native artifact paths.` Spec-to-bite-sized-tasks planner with the repo's `/execute-plan` flow.
- feat(skills): `vendored using-git-worktrees from superpowers.` Isolated worktree or plain branch before executing plans.
- feat(skills): `vendored systematic-debugging from superpowers.` Hypothesis-first debugging; references moved to `references/`.
- feat(skills): `vendored writing-skills from superpowers, examples moved to references.` Meta-skill for authoring/editing skills; references kept under `references/`.
