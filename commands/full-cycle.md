...
5. Execute (default): dispatch the `orchestrator` subagent with the spec and plan paths; it branches, runs executor/reviewer in parallel waves (executors for all ready tasks at once, each reviewer pipelined as its own executor returns), escalates two-strike failures to `oracle`, then runs a structure review (`doc-standardizer` + `code-standardizer`, dispatched concurrently, with quick-fix `executor` passes) and a documentation phase (`documenter` writes the execution report to `docs/artifacts/features/` and updates catalogs, plus writes choices entries to `docs/artifacts/choices/`), commits at boundaries, and returns a final report. Relay that report. If `orchestrator` is unavailable, dispatch `general` with the same instructions; if no subagent dispatch is possible, fall back to step 6.
6. Handoff (only when `handoff` is present, or as the fallback): print the spec and plan paths plus `/execute-plan docs/artifacts/features/<topic>/<file>.md` for a fresh session.
