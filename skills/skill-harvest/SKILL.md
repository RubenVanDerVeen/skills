...
Mines recent opencode sessions for repeated corrections, recurring friction, and unmet needs, then turns approved findings into edits to this skills repo. Instruction-only: every extraction command lives in `references/extraction.md`, run them via shell.
...
1. **State.** Read `~/.skill-harvest-state.json` (ISO 8601 UTC timestamp of last run). Missing file: use a 30-day lookback and say so in the report.
2. **Enumerate.** List sessions newer than the timestamp (command in `references/extraction.md`), oldest first so the backlog drains FIFO and nothing is re-processed. Optional project argument filters by directory substring. Eval-harness dirs (`ai-harness-eval`) and native agent workspaces (`.minimax`) are excluded by default; an explicit project filter overrides this. Cap 40 sessions per run; list the remainder in the report.
...
7. **Update state.** Set the timestamp to the mtime of the newest session processed in that batch (the FIFO cursor). Write even on zero-finding runs. If no sessions were processed, leave the timestamp unchanged.

## Hard rules

- Never bulk-read transcripts; digests only.
- Never auto-apply an edit, even a trivial one.
- Skipped source (e.g. opencode DB unreadable): note it in the report, do not advance the timestamp.
