...
10. **Wire the knowledge graph (graphify)** (medium + large; skip at small tier, and skip when `command -v graphify` finds no CLI): a ~1–2K-token `graphify query` replaces grep-heavy exploration whose output would otherwise be re-billed on every later prompt in the session.
    ...
    4. Keep the **Knowledge graph** section that ships in the medium/large `AGENTS.md` templates. When this step is skipped, delete that section instead: an instruction pointing at a graph that does not exist sends agents down a dead end.
