...
A **spec** documents the design decisions for a feature or change. Captures scope, architecture, tradeoffs, pre/post-deploy checklists. Written by the user (or by Claude via `superpowers:brainstorming`) before implementation begins.
...
- Specs are **not auto-loaded** into `CLAUDE.md`. Claude reads them on demand when asked about historical design choices.
- Specs are committed alongside the code they describe — same commit when practical.

## Plans — `docs/artifacts/plans/`

A **plan** is a step-by-step implementation plan with review checkpoints. Written by the user (or by Claude via `superpowers:writing-plans`) after the spec is approved, before code is written.
...
- Plans are consumed during execution (`superpowers:executing-plans` or `subagent-driven-development`). Once executed, the plan stays as historical record.
...
- Repo-structure audits, standards-compliance audits, and project-structure reviews go here — **not** under `claude/`.
...
1. **Brainstorm** (`superpowers:brainstorming` skill) → outputs a spec in `docs/artifacts/specs/`.
2. User reviews spec, approves.
3. **Write plan** (`superpowers:writing-plans` skill) → outputs a plan in `docs/artifacts/plans/` referencing the spec.
4. User reviews plan, approves.
5. **Execute** (`superpowers:executing-plans` or `subagent-driven-development` skill) → consumes plan.
...
- `docs/artifacts/` files are **not** `@import`-ed into `CLAUDE.md`. They are project history, not session context.
- Reference them on demand: "What was the original design for X?" → Claude greps `docs/artifacts/specs/` for the matching topic and reads that file.
- If a critical decision in a spec needs to be remembered cross-session (e.g. "we explicitly chose A over B because of constraint C"), capture that fact in a memory entry (`project_<topic>.md` at `~/.claude/projects/<slug>/memory/`) **as well as** keeping the full spec in `docs/artifacts/specs/`.

## Anti-patterns

- Putting reviews in `claude/reviews/`. Reviews are committed project history — `docs/artifacts/reviews/`. `claude/` is for context files, not artefacts.
