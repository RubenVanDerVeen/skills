...
| `orchestrator` | all | `MiniMax-M3` | Executes approved plans: dispatches executor/reviewer work in parallel waves (ready-set dispatch), pipelines each reviewer against its own task's commit, oracle on two-strike failures, commits at boundaries. Session agent for standalone `/execute-plan`, and dispatchable by the planner for single-pass `/full-cycle` |
...
2. At the end of the pipeline the planner dispatches the `orchestrator` subagent (mode `all`) in the same run, instead of stopping at a handoff. The orchestrator branches as `<type>/<plan-slug>` (`feat` for features, `fix` for bug fixes), commits the plan and spec first, then runs its 8-step loop: parallel waves of `executor` tasks (plans carry `**Depends:**` markers; tasks without them run serially) with a `reviewer` pipelined per task as each executor returns, `oracle` on two-strike failures, `doc-standardizer` + `code-standardizer` structure review run concurrently with quick-fix `executor` passes, then `documenter` writes the execution report and updates catalogs. It commits at task boundaries, often works in a worktree, and returns the final report for the planner to relay.
