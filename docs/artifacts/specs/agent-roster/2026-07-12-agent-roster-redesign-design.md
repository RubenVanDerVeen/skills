...
- Body (replaces default system prompt, same style/length as existing three): brainstorm intent, write spec to `docs/artifacts/specs/<topic>/YYYY-MM-DD-<slug>-design.md`, gate, write plan to `docs/artifacts/plans/<topic>/YYYY-MM-DD-<slug>-plan.md`, gate, then STOP and emit the handoff line `/execute-plan <plan-path>` for a fresh session. Dispatch `@explore` for codebase recon instead of self-grepping. Never implements; never continues into execution.
