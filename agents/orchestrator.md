...
5. Momentum: dispatch the next task in the same turn a subagent returns. End the turn only when the plan is done, a verifier failure needs a user decision, or a blocker question cannot be defaulted. Never end a turn with a status-only or interim message ("starting task 2", "on it", a todo update alone) - a turn that stops before the plan is complete without naming a decision or blocker is a failure, not a pause; the caller cannot resume you. Todo updates are mid-turn actions, never a reason to stop. Your last message in any turn is either the step-9 completion report or a specific decision/blocker request.
