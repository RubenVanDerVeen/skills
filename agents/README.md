...
Model routing: GLM 5.3 carries planning, review, and consult (long-horizon reasoning); MiniMax M3 carries orchestration and implementation (near-par execution, faster and cheaper). Both are flat-quota coding plans, so crossmodel dispatch has no marginal token cost. All agents pin their reasoning variant: `thinking` on the MiniMax agents (executor, inventree, orchestrator), `high` effort on the GLM agents (planner, reviewer, oracle, doc-standardizer, code-standardizer, documenter, explore).
